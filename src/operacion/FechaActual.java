...
public class FechaActual implements Operacion{
	...
	public String calcular(Pedido pedido) {
		...
		String regex_DiaActual = ".*(que hora es|la hora|que dia es|la fecha|fecha es hoy|dia de la semana es|dia es de la semana).*";
		Pattern pattern_DiaActual = Pattern.compile(regex_DiaActual, Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
		Matcher matcher = pattern_DiaActual.matcher(pedido.getMensaje());
		if(matcher.matches()) {
			if(matcher.group(1).contains("que hora es") || matcher.group(1).contains("la hora")) {
				return pedido.getNameUsuario() + " son las " + diaActual.hora(diaActual.formato_hora[0]);	
			}
			if(matcher.group(1).contains("que dia es") || matcher.group(1).contains("la fecha") || matcher.group(1).contains("fecha es hoy")) {
				return pedido.getNameUsuario() + " hoy es " + diaActual.fechaToString(diaActual.formato_fecha[0]);
			}
			if(matcher.group(1).contains("dia de la semana es") || matcher.group(1).contains("dia es de la semana")) {
